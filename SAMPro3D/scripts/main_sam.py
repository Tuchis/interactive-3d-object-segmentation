...
from sam_utils.vis_utils import *
...
def perform_3dsegmentation(xyz, keep_idx, scene_output_path, npy_path, args):
    ...
    num_ins = 2 if args.neg_bg else 1
    ...
    for i, (npy_file) in enumerate(tqdm(npy_path)):
        ...
        try:
            ...
            print(f"File {npy_file} not found, probably due to frame interval skip")
            continue
        # Reshape the masks_data to the shape of frame
        # Comment that part, as it breaks the code
        # print(f"masks_data shape: {masks_data.shape}")
        # if args.sam2:
        #     masks_data = masks_data.reshape(-1, 480, 640)
        # print(f"masks_data shape after reshape: {masks_data.shape}")
        corre_3d_ins_data = torch.from_numpy(np.load(os.path.join(scene_output_path, 'corre_3d_ins_npy', npy_file))).to(device)  # the valid (i.e., has mapped pixels at the current frame) prompt ID  in the original 3D point cloud of initial prompts
        data = MaskData(
                masks=masks_data,
                iou_preds=iou_preds_data,
        ...
        # We have to scale masks shape to the same as the depth camera image
        # Right now masks_logits have shape (1440, 1920)
        # We need to scale it to (192, 256)
        ...
        if masks_logits.ndim == 4:
            ...
            print(f"masks_logits shape after squeeze: {masks_logits.shape}")
        if os.path.exists(depth_intrinsic_path):
            masks_logits = F.interpolate(masks_logits.unsqueeze(0), size=(192, 256), mode='bilinear', align_corners=False).squeeze(0)
        masks = masks_logits > 0.

        ins_idx_all = [0, 1] if args.neg_bg else [0]
        ...
        for index, (mask) in enumerate(masks):  # iterate over each mask area segmented by different prompts
            ...
            # This line uses the 2D coordinates from the mapping (x,y coordinates stored in mapping[:, 0] and mapping[:, 1])
            # to index into the 2D mask and get the corresponding mask values for each 3D point
            mask_2d_3d = mask[mapping[:, 0], mapping[:, 1]]
            # This line multiplies the mask values by mapping[:, 2], which contains a binary flag (0 or 1)
            # indicating whether a valid mapping exists for each point. This effectively zeroes out
            # mask values for points that don't have a valid 2D-3D mapping
    ...
    pt_pred_abs[pt_pred_abs == 1] = -1

    low_pt_idx_mean = np.where(max_score <= args.segmentation_threshold)[0]  # assign ins_label=-1 (unlabelled) if its score=0 (i.e., no 2D mask assigned)
    pt_score_mean[low_pt_idx_mean] = 0.
    pt_pred_mean[low_pt_idx_mean] = -1

    low_pt_idx_abs = np.where(max_score_abs <= args.segmentation_threshold)[0]
...
def get_args():
    ...
    parser.add_argument("--sam2_video", action='store_true', help="Use SAM2 video or not.")
    # arguments for 3D segmentation:
    # Neggative background - agigns not assigned points to a negative background class
    parser.add_argument('--segmentation_threshold', type=float, default=0.0, help='Threshold for the score of the point to be assigned to a class.')
    # Threshold for the score of the point to be assigned to a class
    parser.add_argument('--neg_bg', action='store_true', help='Whether to assign points to a negative background class.')
...
if __name__ == "__main__":
    ...
    init_prompt, _ = load_ply(args.prompt_path)
    ...
    points_npy_path = natsorted(os.listdir(f"{scene_output_path}/points_npy"))
    iou_preds_npy_path = natsorted(os.listdir(f"{scene_output_path}/iou_preds_npy"))
    masks_npy_path = natsorted(os.listdir(f"{scene_output_path}/masks_npy"))
    corre_3d_ins_npy_path = natsorted(os.listdir(f"{scene_output_path}/corre_3d_ins_npy"))
    if not (args.sam2 and args.sam2_video):
    ...
    # keep_idx = torch.arange(0, 2).to(device)
    ...
    ps.print_stats(50)  # Print only top 10 entries
    print(s.getvalue())