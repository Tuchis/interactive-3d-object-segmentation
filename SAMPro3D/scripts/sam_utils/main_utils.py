...
def load_ply(ply_path):
    if not os.path.exists(ply_path):
        print(f"PLY file does not exist at {ply_path}")
        return np.array([]), np.array([])
    # If ply_path ends with _vh_clean_2.ply, check if it has cached npy files
    if ply_path.endswith('_vh_clean_2.ply'):
        xyz_path = ply_path.replace('.ply', '_xyz.npy')
        rgb_path = ply_path.replace('.ply', '_rgb.npy')
        if os.path.exists(xyz_path) and os.path.exists(rgb_path):
    ...
    # If ply_path ends with _vh_clean_2.ply, cache xyz and rgb to npy files
    if ply_path.endswith('_vh_clean_2.ply'):
        np.save(ply_path.replace('.ply', '_xyz.npy'), xyz)
        np.save(ply_path.replace('.ply', '_rgb.npy'), rgb)
...
def compute_mapping(points, data_path, scene_name, frame_id):  
    ...
    depth_intrinsic_path = os.path.join(data_path, scene_name, 'intrinsics', f'{frame_id}.txt')
    # print(f"Depth intrinsic path: {depth_intrinsic_path}")
    if os.path.exists(depth_intrinsic_path):
        # Load the intrinsic
        rgb_intrinsic = np.loadtxt(depth_intrinsic_path)
        ...
        # Load the intrinsic
        depth_intrinsic = np.loadtxt(f"{data_path}/intrinsics.txt")
    # Load the intrinsic matrix
    # depth_intrinsic = np.loadtxt(os.path.join(data_path, 'intrinsics.txt'))
    
    # Construct paths
    depth_png_path = os.path.join(data_path, scene_name, 'depth', f'{frame_id}.png')
    depth_npy_path = os.path.join(data_path, scene_name, 'depth', f'{frame_id}.npy')
    ...
    # world_to_camera_cache_path = os.path.join(data_path, scene_name, 'cache', f'world_to_camera_{frame_id}.npy')
    # if os.path.exists(world_to_camera_cache_path):
    #     world_to_camera = np.load(world_to_camera_cache_path)
    # else:
    world_to_camera = np.linalg.inv(pose)
    # os.makedirs(os.path.dirname(world_to_camera_cache_path), exist_ok=True)
    # np.save(world_to_camera_cache_path, world_to_camera)
...
def get_labels(scene_name):
    agile_3d_scans_path = "/home/kuzhum/sam/promptable-3d-object-segmentation/AGILE3D/data/ScanNetPP_short/scans"
    scene_path = os.path.join(agile_3d_scans_path, scene_name + '.ply')
    if not os.path.exists(scene_path):
        return False
    # Load the PLY file
    ply_file = PlyData.read(scene_path)
    vertex = ply_file["vertex"].data
